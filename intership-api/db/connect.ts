...
export class DbModel {
  ...
  private static async getId(type: string) {
    ...
    //return esence.length + 1; // было
    return Math.max(...esence.map((elem) => elem.id)) + 1;
  }
  ...
  public static async getEmployeeById(id: number) {
    let result = await db.get("employee").filter({ id_division: id }).value();
    return result;
  }
  ...
  public static async deleteEsense(id: number, type: string) {
    let deleted = await db.get(type).remove({ id: id }).write();
    return deleted;
  }
